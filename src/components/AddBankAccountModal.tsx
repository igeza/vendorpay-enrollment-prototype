...
import type { BankAccountRecord } from "./BankAccountDetailsModal"
...
const AVAILABLE_BANKS = ["1002 Wells Fargo", "1010 Bank of America", "1011 Citibank", "1012 Regions Bank", "1013 Capital One"]
const OWNER_TYPES = ["Business", "Personal"]
const PAYER_NAMES = ["Company Name", "Premiere PMC", "Property Name"]

const EMPTY_FORM: BankAccountRecord = {
  bankName: "",
  routingNumber: "",
  accountNumber: "",
  ownerName: "",
...
export function AddBankAccountModal({ onClose, onAdd }: { onClose: () => void; onAdd: (account: BankAccountRecord) => void }) {
  const [form, setForm] = useState<BankAccountRecord>(EMPTY_FORM)
  ...
  const canAdd = form.bankName && form.routingNumber && form.accountNumber
        ...
        <div className="flex flex-col gap-lg p-md">
          <div className="flex items-end gap-md">
            <div className="flex-1">
              <Dropdown label="Bank Account" required options={AVAILABLE_BANKS} value={form.bankName} onChange={(v) => set("bankName", v)} />
            </div>
